...
def initParams():
    ...
    parser.add_argument("--out_fold", type=str, help="output folder",default='mixup_0.1/')
...
def train(args):
    ...
    for epoch_num in range(args.num_epochs):
        ...
        for i, (lfcc,cqt,label,fakelabel) in enumerate(tqdm(trainDataLoader)):
            ...
            index = torch.randperm(len(label))
            label_b = label[index]
            ...
            # calculate fcloss
            lfcc = lam * lfcc + (1 - lam) * lfcc[index, :]
            _, preds_lfcc = resnet_lfcc(lfcc)
            fcloss_lfcc = lam * criterion(preds_lfcc, label) + (1 - lam) * criterion(preds_lfcc, label_b)
            ...
            # calculate fcloss
            cqt = lam * cqt + (1 - lam) * cqt[index, :]
            _, preds_cqt = resnet_cqt(cqt)
            fcloss_cqt = lam * criterion(preds_cqt, label) + (1 - lam) * criterion(preds_cqt, label_b)
