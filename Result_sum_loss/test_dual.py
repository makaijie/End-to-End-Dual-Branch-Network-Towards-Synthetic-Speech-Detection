...
def test_model(lfcc_path,cqt_path, device):
    ...
    with open(os.path.join('Result_sum_loss', 'checkpoint_cm_score.txt'), 'w') as cm_score_file:
            ...
            for j in range(labels.size(0)):
                cm_score_file.write(
                    '%s A%02d %s %s\n' % (filename[j], type[j].data,
                                          ...
                                          score[j].item()))
    eer_cm, min_tDCF = compute_eer_and_tdcf(os.path.join('Result_sum_loss', 'checkpoint_cm_score.txt'),'')
    return eer_cm

def test(model_dir, device):

    lfcc_path = "anti-spoofing_lfcc_model_100.pt"
    cqt_path = "anti-spoofing_cqt_model_100.pt"
...
if __name__ == "__main__":
    ...
    parser.add_argument('-m', '--model_dir', type=str, default="Result_sum_loss/")
